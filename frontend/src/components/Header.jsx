...
const NAV_TRANSLATIONS = {
  en: {
    ...
    gear: 'Gear', contact: 'Contact', hrZones: 'HR Zones', podcasts: 'Podcasts', openclaw: 'OpenClaw Daily', fitness: 'Fitness Tech Podcast',
  },
  de: {
    ...
    gear: 'Ausrüstung', contact: 'Kontakt', hrZones: 'HF-Zonen', podcasts: 'Podcasts', openclaw: 'OpenClaw Podcast', fitness: 'Fitness-Tech-Podcast',
  },
  es: {
    ...
    gear: 'Equipo', contact: 'Contacto', hrZones: 'Zonas FC', podcasts: 'Podcasts', openclaw: 'Podcast OpenClaw', fitness: 'Podcast Fitness Tech',
  },
  pt: {
    ...
    gear: 'Equipamento', contact: 'Contato', hrZones: 'Zonas FC', podcasts: 'Podcasts', openclaw: 'Podcast OpenClaw', fitness: 'Podcast Fitness Tech',
  },
  hi: {
    ...
    gear: 'गियर', contact: 'संपर्क', hrZones: 'HR ज़ोन', podcasts: 'पॉडकास्ट', openclaw: 'OpenClaw पॉडकास्ट', fitness: 'फिटनेस टेक पॉडकास्ट',
  },
};
...
export default function Header() {
  ...
  const navLinks = useMemo(() => ([
    ...
    { name: t.openclaw, href: `${podcastBase}/podcasts/openclaw/`, icon: '🎙️' },
          ...
          <nav className="hidden lg:flex items-center gap-3 xl:gap-5 mx-4 min-w-0 overflow-x-auto">
            {navLinks.map((link) => link.hasDropdown ? (
              <div key={link.name} className="relative" ref={blogDropdownRef} onMouseEnter={() => setIsDesktopBlogOpen(true)} onMouseLeave={() => setIsDesktopBlogOpen(false)}>
                <a href={link.href} onClick={(e) => { e.preventDefault(); setIsDesktopBlogOpen((o) => !o); }} className={`text-sm font-medium transition-colors flex items-center gap-1.5 cursor-pointer ${pathname === link.href || (pathname.startsWith(link.href) && link.href !== '/') ? 'text-white border-b border-blue-500 pb-0.5' : 'text-neutral-300 hover:text-white'}`}>
