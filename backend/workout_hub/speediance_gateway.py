...
class SpeedianceGateway:
    ...
    def login(self, email: str, password: str) -> dict:
        ...
        if verify.get("isExist") is False:
            raise SpeedianceProviderError("Speediance account does not exist")
        if verify.get("hasPwd") is False:
            raise SpeedianceProviderError("Set a password in the official Speediance app first")
        try:
        ...
        if not data.get("token") or not data.get("appUserId"):
            raise SpeedianceProviderError("Speediance login did not return a session")
    ...
    def save_workout(self, name: str, exercises: list[dict]) -> dict:
        ...
        for exercise in exercises:
            ...
            for index, item in enumerate(exercise.get("sets") or []):
                ...
                unit = str(item.get("unit") or "reps").lower()
