...
| 6 | **IN PROGRESS** | Replace Anthropic's generic next-step block | Medium | 66 viewers and zero clicks despite 43.5% qualified engagement | High | Low | Replaced the AgentStack intro route with a refund-evidence checklist CTA and added the checklist in `PriorityEntryPath.astro` and `anthropic-refund-scam.md`; live verification pending |
| 7 | **IN PROGRESS** | Test a demand-led homepage feature | Medium | Hero meets threshold; featured BJJ topic conflicts with search demand | Medium | Low | Pinned the canonical Gym Monster 1 vs 2 vs 2S guide in the hero under layout `home-2026-08-18-gym-monster-a`; collect one complete week before deciding the winner |
| 8 | **IN PROGRESS** | Test Comparisons in primary navigation | Medium | About had 217 exposed viewers and zero clicks | Medium | Low | Replaced primary slot 7 with localized Comparisons under schema `nav-2026-08-18-comparisons-a`; About remains in the footer and hero while the test collects one complete week |
